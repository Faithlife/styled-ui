...
import { rowMasonryList } from './util';

import { AutoSizedRowMasonry, Box } from '../../index';

<Meta
	title="Layout Components/Row Masonry"
	component={AutoSizedRowMasonry}
	argTypes={{
		gapSize: { control: { type: 'number' } },
		targetRowHeight: { control: { type: 'number' } },
		minRowItems: { control: { type: 'number' } },
		maxRowItems: { control: { type: 'number' } },
		getItemAspectRatio: { action: 'getItemAspectRatio' },
		renderItem: { action: 'renderItem' },
...
<Canvas>
	<Story
		name="Row Masonry Example"
		args={{ gapSize: 12, targetRowHeight: 100, minRowItems: 1, maxRowItems: 4 }}
	>
		{args => (
			<Box maxWidth={400}>
				<AutoSizedRowMasonry
					{...args}
					items={rowMasonryList}
					getItemAspectRatio={x => x.aspectRatio}
					renderItem={x => <Box flex={1} backgroundColor="blue2" />}
				/>
			</Box>
		)}
	</Story>
