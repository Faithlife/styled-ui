...
import { Button, CloseButton, Text, Paragraph } from '@faithlife/styled-ui';
...
export class LargeGroupSelector extends React.Component {
	static propTypes = {
		...
		localizedResources: PropTypes.shape({
			...
			contactAdmin: PropTypes.string,
			visitGroup: PropTypes.string,
			groupTypeRequirement: PropTypes.string,
			membershipRequired: PropTypes.string,
			period: PropTypes.string,
			isInvited: PropTypes.string,
			isAnAdmin: PropTypes.string,
			isAModerator: PropTypes.string,
			isAnObserver: PropTypes.string,
			isAFollower: PropTypes.string,
			isAMember: PropTypes.string,
			notAMember: PropTypes.string,
			invitedText: PropTypes.string,
			adminText: PropTypes.string,
			moderatorText: PropTypes.string,
			observerText: PropTypes.string,
			followerText: PropTypes.string,
			memberText: PropTypes.string,
			notText: PropTypes.string,
			membershipRequiredTemplate: PropTypes.string,
		}),
	};
	...
	static defaultProps = {
		...
		localizedResources: {
			...
			createGroup: 'Create',
			contactAdmin: 'Contact a group administrator to request access.',
			visitGroup: 'Visit the group settings page to change.',
			groupTypeRequirement: 'This group type must be set to ',
			membershipRequired: ' membership is necessary to perform this action.',
			period: '.',
			isInvited: 'You are invited.',
			isAnAdmin: 'You are an admin.',
			isAModerator: 'You are a moderator.',
			isAnObserver: 'You are an observer.',
			isAFollower: 'You are a follower.',
			isAMember: 'You are a member.',
			notAMember: 'You are not a member.',
			invitedText: 'invited',
			adminText: 'admin',
			moderatorText: 'moderator',
			observerText: 'observer',
			followerText: 'follower',
			memberText: 'member',
			notText: 'not',
			membershipRequiredTemplate: '%membershipKinds% membership required',
		},
	};
				...
				<Styled.CreateGroupWrapper fixed={createGroupFixed}>
					<Styled.CreateGroupBackground scrollWidthDelta={scrollWidthDelta}>
						...
						<Styled.CreateGroupButtonWrapper>
							...
							>
								{localizedResources.createGroup}
							</Button>
						</Styled.CreateGroupButtonWrapper>
				...
				>
						...
						<>
							...
							<Modal.Content width={['100vw', 375]}>
								<Text display="inline-block">
									...
									<span>{localizedResources.membershipRequired}</span>
								</Text>
								<Text>{localizedResources.contactAdmin}</Text>
							</Modal.Content>
							<Modal.Footer>
								<Modal.FooterButtons
					...
					{modalContent === 'change' && (
						<>
							...
							<Modal.Content width={['100vw', 375]}>
								<Paragraph>
									{localizedResources.groupTypeRequirement}
									<Text color="flGray" fontWeight="bold">
										{formattedGroupLevels}
									</Text>
									{localizedResources.period}
								</Paragraph>
								<Paragraph>{localizedResources.visitGroup}</Paragraph>
							</Modal.Content>
							<Modal.Footer>
								<Modal.FooterButtons
									cancelButton={{ onClick: this.resetModalState, text: 'Cancel' }}
